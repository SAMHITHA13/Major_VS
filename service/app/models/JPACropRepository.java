...
public class JPACropRepository implements CropRepository {
            ...
            try {
                js = (ObjectNode) new ObjectMapper().readTree("{" +
                        ...
                        "\"harvestedQuantity\" : \"" + toPay.get(i).harvestedQuantity + "\"," +
                                "\"totalPayable\" : \"" + toPay.get(i).totalPayable + "\""+
                        "}");
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private Stream<Object[]> viewMap(EntityManager em, String stateName, String cropName, Integer tp) {
        ...
        List<Object[]> Crops1 = em.createQuery("select count(*),c.location, sum(c.area), sum(c.quantitymax), c.latitude, c.longitude from Crop c where lower(c.stateLocation)=:stateName and lower(c.name)=:cropName and TIMESTAMPDIFF(month, c.starttime, c.endtime) <=: tp group by c.location, c.latitude, c.longitude ", Object[].class)
                .setParameter("stateName",stateName)
                .setParameter("cropName",cropName)
                .setParameter("tp",tp)
                .getResultList();
        /*for (Object[] obj : Crops1){
    ...
    private Stream<Object[]> cropsList(EntityManager em) {
        List<Object[]> Crops = em.createQuery("select c.name from Crop c group by c.name", Object[].class).getResultList();
        return Crops.stream();
    }
