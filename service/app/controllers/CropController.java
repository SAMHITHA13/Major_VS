...
public class CropController extends Controller {
    ...
    public CompletionStage<Result> viewMap(String stateName, String cropName, Integer tp) {
        return cropRepository.viewMap(stateName, cropName, tp).thenApplyAsync(mapStream -> {
            //System.out.println(toJson(mapStream.collect(Collectors.toList())));
            return ok(toJson(mapStream.collect(Collectors.toList())));
        }, ec.current());
    }

    public CompletionStage<Result> getCropsList() {
        return cropRepository.cropsList().thenApplyAsync(cropStream -> {
            return ok(toJson(cropStream.collect(Collectors.toList())));
        }, ec.current());
    }
}
